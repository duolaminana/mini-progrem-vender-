...
Page({
  ...
  cutRebate (){
	  ...
	  if(this.data.rebateIcon){
		this.calculateOrderParams(this.data.rebate)
	  }else{
		this.calculateOrderParams(0)
	  }
  },
  ...
  calculateRrealPrice (rebate){
	if(this.data.show_totalPrice <= 0){
		...
		{
			this.setData({
				show_realPrice: this.data.show_totalPrice,
				showRebate : false
			})
		}
	}
  ...
  initOpen (){
	...
	calculateOrderParams (rebate){
		...
		this.calculateRrealPrice(rebate)
		...
		this.data.q = {
			...
			"preferentialPrice": this.data.haveXM ? this.data.showRebate ? this.data.rebateIcon ? this.data.rebate : null : null : null
