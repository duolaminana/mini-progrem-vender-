...
Page({
  ...
  getBackcash () {
    postRebateList(this.data.pageNum + 1).then(res=>{
      console.log('获取返现信息接口:', res.result)
      this.data.pageNum += 1
      this.setData({
        total: res.result.remainingProfitofit || 0,
        thisList: this.data.thisList.concat(res.result.rebate.list || [])
      })
    })
  },

  getIntegral () {
    postIntegraList(this.data.pageNum + 1).then(res=>{
      console.log('获取积分信息接口:', res.result)
      this.data.pageNum += 1
      this.setData({
        total: res.result.allIntegrals || 0,
        thisList: this.data.thisList.concat(res.result.integral.list || [])
      })
    })
  },
