...
module.exports = class CartStockApi{//定义了一个名字为Person的类
    constructor(data){//constructor是一个构造方法，用来接收参数
        this.data = data ? data : [];
		this.totalPrice = 0;  // 零售价
		this.totalActivityPrice = 0; // 活动价
		this.totalSubPrice = 0; // 总优惠价
		this.totalArticle = 0; // 总物件
		this.type = []; // 购物车商品属性
    }
	...
	_calculate(){ // 计算总和 activityPrice == null时特殊处理
		...
		this.totalArticle = 0
		for(let item of this.data){
			this.totalActivityPrice += (item.activityPrice == null ? item.actualPrice : item.activityPrice) * item.dev_custom_count
			this.totalPrice += item.actualPrice * item.dev_custom_count
			this.totalArticle += item.dev_custom_count
		}
		this.totalSubPrice = this.totalPrice - this.totalActivityPrice
	}
	AddGoodsType(obg){ // 添加购物车商品属性之类的东东.幸哉
		this.type.push(obg)
	}
}