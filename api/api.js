...
export function getGoodsHotData() {
  return request.get("/position/miniProgram/queryHotProduct", {
    latitude: getApp().globalData.User_location[0],
    longitude: getApp().globalData.User_location[1]
  }, {
    noAuth: true
  });
}
...
export function queryNearbyDevice(data = {}) {
  return request.post("/position/miniProgram/findNearByMachine", {
    laytitude: getApp().globalData.User_location[0],
    longitude: getApp().globalData.User_location[1],
    memberId: getApp().globalData.wxUserInfo.id,
    ...data
  }, {
...
export function findProductChannel(data) {
  return request.post('/product/productChannel/findProductChannel', data, {
    noAuth: true
  });
}
...
export function returnMoney(data) {
  return request.get(`/pay/getRebate`, data, {
    noAuth: true
  });
}
...
export function useReturnMoney(code) {
  return request.post('/order/memberProfit/findMemberProfitByMemberId?memberId=' + getApp().globalData.wxUserInfo.id + '&channelId=' + (code || getApp().globalData.channelId), {
    memberId: getApp().globalData.wxUserInfo.id,
    channelId: code || getApp().globalData.channelId
  }, {
    noAuth: true
  });
}
...
export function wxAppSmsCode(phone) {
  return request.post(`/auth/loginUser/wxAppSmsCode?memberId=${getApp().globalData.wxUserInfo.id}&phone=${phone}`, {}, {
    noAuth: true
  })
}
...
export function queryMachine(code) {
  return request.get(`/position/miniProgram/queryMachine`, {
    machineCode: code
  }, {
    noAuth: true
  })
...
export function getGoodsyn(data) {
  return request.get(`/position/machineRoad/goodsyn`, data, {
    noAuth: true
  })
}
...
export function witnessCheck(data) {
  return request.upload(`/face/witnessCheck`, data, {
    noAuth: true
  })
}
...
export function checkFace(data) {
  return request.upload(`/face/checkFace`, data, {
    noAuth: true
  })
}
...
export function outStatus(data) {
  return request.get(`/mqtt/miniProgram/outStatus`, data, {
    noAuth: true
  })
}
...
export function getCodeClaimGoods() {
  return request.get(`/order/wxAppApi/queryPendingProduct`, {
    memberId: getApp().globalData.wxUserInfo.id
  }, {
    noAuth: true
  })
...
export function queryOrderDetailes(data) {
  return request.post(`/order/wxAppApi/queryOrderDetailes`, {
    memberId: getApp().globalData.wxUserInfo.id,
    pageNum: data,
    pageSize: 10
  }, {
 ...
 * 获取环保袋限免购买过的数量
 */
export function findMemberFreeProduct(str, channelId) {
  return request.post(`/order/wxAppApi/findMemberFreeProduct?memberId=${getApp().globalData.wxUserInfo.id}&productCodes=${str}&channelId=${channelId}`, {
    memberId: getApp().globalData.wxUserInfo.id,
    productCodes: str,
    channelId
  }, {
...
export function updateNickName(name) {
  return request.post(`/order/wxAppApi/updateNickName`, {
    id: getApp().globalData.wxUserInfo.id,
    "nickName": name
  }, {
    noAuth: true
  })
...
export function getFriendList() {
  return request.get(`/order/wxAppApi/queryFriendList`, {
    memberId: getApp().globalData.wxUserInfo.id
  }, {
    noAuth: true
  })
